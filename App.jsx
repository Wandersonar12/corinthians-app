...
import { fetchLiveMatch, fetchFixtures, fetchMatchStats, fetchSquad, fetchStandings, fetchNews } from "./api";
import { CORINTHIANS_ID } from "./config";
...
function isCori(id){return id===CORINTHIANS_ID}
...
export default function App() {
  ...
  const TABS = [["noticias","Notícias"],["jogos","Jogos"],["tabela","Tabela"],["elenco","Elenco"],["momentos","Momentos"],["assistente","Assistente"]];
    ...
    <>
      ...
      {tab==="jogos" && (
        <div className="page">
          ...
          {liveMatch?.isLive && (
            <div style={{background:"#1a0000",border:"1px solid #cc0000",borderRadius:10,padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:10}}>
              <span style={{background:"#cc0000",color:"#fff",fontFamily:"'Montserrat',sans-serif",fontWeight:800,fontSize:".65rem",letterSpacing:"2px",padding:"3px 8px",borderRadius:3,animation:"pulse 2s infinite"}}>● AO VIVO</span>
              <span style={{fontFamily:"'Montserrat',sans-serif",fontWeight:700,fontSize:".85rem"}}>{liveMatch.teams?.home?.name} {liveMatch.goals?.home} - {liveMatch.goals?.away} {liveMatch.teams?.away?.name}</span>
              <span style={{color:"var(--gold)",fontSize:".75rem",fontWeight:700,marginLeft:"auto"}}>{liveMatch.status?.elapsed}'</span>
            </div>
          )}
